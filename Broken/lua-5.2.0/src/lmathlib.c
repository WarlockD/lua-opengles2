...
#undef PI
#define PI (LUAI_UACNUMBER)(3.14159265358979323846)
#define RADIANS_PER_DEGREE (LUAI_UACNUMBER)(PI/180.0)
...
static int math_random (lua_State *L) {
  ...
  switch (lua_gettop(L)) {  /* check number of arguments */
    ...
    case 1: {  /* only upper limit */
      ...
      lua_pushnumber(L, l_tg(floor)(r*u) + 1.0f);  /* int in [1, u] */
      break;
    }
...
LUAMOD_API int luaopen_math (lua_State *L) {
  ...
  lua_pushnumber(L, PI);
  lua_setfield(L, -2, "pi");
  lua_pushnumber(L, (LUAI_UACNUMBER)HUGE_VAL);
